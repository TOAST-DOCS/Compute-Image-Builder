...
Valkey가 사용할 ip에 대한 값입니다. 서버 외부에서 Valkey 인스턴스로의 접근을 허용하려면 해당 파라미터에 private ip를 추가해야 합니다. private ip는 `hostname -I` 커맨드로 확인할 수 있습니다.
...
<a id="automatic-ha-configuration-script"></a>
### 자동 HA 구성 스크립트
NHN Cloud의 Valkey 인스턴스는 자동으로 HA 환경을 구성해 주는 스크립트를 제공합니다. 스크립트는 반드시 **설치 직후의 신규 인스턴스**에서만 사용할 수 있으며, valkey.conf에서 설정값을 변경한 경우에는 사용할 수 없습니다.
...
##### 보안 그룹 설정
Valkey 인스턴스간의 통신을 위해 보안 그룹(**Network** > **Security Groups**) 설정이 필요합니다. 아래 규칙으로 보안 그룹을 생성한 뒤 Valkey 인스턴스에 적용하세요.
