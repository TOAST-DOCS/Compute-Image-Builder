...
**이미지 템플릿 이름**을 입력하고, OS는 **linux - Ubuntu - Server xx.xx LTS**를 선택하고, **최소 블록 스토리지**는 **70GB** 이상으로 지정합니다.
...
로그 파일의 경로는 `/var/log/slurm/` 아래로 지정해야 합니다. 다른 경로를 지정하려면 설정 파일에 경로를 명시하고 해당 디렉터리의 소유자를 `SlurmUser` 설정값과 동일하게 설정해야 합니다.
